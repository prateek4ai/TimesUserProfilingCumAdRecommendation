...
def ingest_and_validate():
    ...
    events_df.write_parquet('outputs/raw_events.parquet')
    items_df.write_parquet('outputs/raw_items.parquet')
...
# Load raw data
events_df = pl.read_parquet('outputs/raw_events.parquet')
items_df = pl.read_parquet('outputs/raw_items.parquet')
...
def create_content_embeddings(items_df, embedding_dim=50):
    ...
    try:
        ...
        tfidf_matrix = tfidf.fit_transform(combined_text.fill_null(''))
...
# Save feature stores
user_store.write_parquet('outputs/user_feature_store.parquet')
item_store.write_parquet('outputs/item_feature_store.parquet')
training_data.write_parquet('outputs/training_data.parquet')
...
training_data = pl.read_parquet('outputs/training_data.parquet')
    ...
    def fit_transform(self, df):
        ...
        for col in self.categorical_features:
            if col in df_pd.columns:
                le = LabelEncoder()
                categorical_encoded[col] = le.fit_transform(df_pd[col].fill_null('unknown').astype(str))
                self.label_encoders[col] = le
        
        # Handle numerical features
        numerical_data = df_pd[self.numerical_features].fill_null(0)
...
def train_model(model, train_loader, val_loader, epochs=10, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        if val_metric > best_metric:
            ...
            torch.save(model.state_dict(), 'outputs/best_wide_deep_model.pt')
...
print(f"💾 Model saved to: outputs/best_wide_deep_model.pt")
...
user_store = pl.read_parquet('outputs/user_feature_store.parquet')
training_data = pl.read_parquet('outputs/training_data.parquet')
...
def train_sequence_model(model, train_loader, val_loader, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), 'outputs/best_din_model.pt')
...
try:
    ...
    print(f"💾 Model saved to: outputs/best_din_model.pt")
...
items_df = pl.read_parquet('outputs/raw_items.parquet')
events_df = pl.read_parquet('outputs/raw_events.parquet')
item_store = pl.read_parquet('outputs/item_feature_store.parquet')
...
class ContentSimilarityEngine:
    ...
    def fit(self):
        ...
        # Combine text features
        combined_text = (
            self.items_df['title'].fill_null('') + ' ' + 
            self.items_df['description'].fill_null('') + ' ' + 
            self.items_df['category_l1'].fill_null('') + ' ' + 
            self.items_df['category_l2'].fill_null('')
        )
...
training_data = pl.read_parquet('outputs/training_data.parquet')
cold_predictions = pl.read_parquet('outputs/cold_item_predictions.parquet')
item_store = pl.read_parquet('outputs/item_feature_store.parquet')
...
if all_recommendations:
    ...
    results_df.write_parquet('outputs/final_recommendations.parquet')
...
# Load final results
final_recommendations = pl.read_parquet('outputs/final_recommendations.parquet')
cold_predictions = pl.read_parquet('outputs/cold_item_predictions.parquet')
